...
class RimrocTestSimulation extends Simulation {
	...
	val processScenario = scenario("Process scenario").repeat(10) { exec(ProcessSequence.process) }
	...
	setUp(
		processScenario.inject(atOnceUsers(3)),
//		iprocessScenario.inject(atOnceUsers(1))
		jobScenario.inject(atOnceUsers(3))
