 ...
 * 3/2/2012
 * 
 * DH: This class is locally overridden to get rid of a nasty memory leak problem which was caused by the Vector
 * pathsMatchingLocationPattern which was initialized where declared and never cleared.
 */
    ...
    public GlobusResource[] getResources(String locationPattern) {
        ...
        if (locationPattern.startsWith("classpath:")) {
            ...
            if ((locationPattern.replaceFirst("file:", "").compareTo(getPathUntilWildcard(locationPattern.replaceFirst("file:", ""), true))) == 0) {//Check to see if the pattern is not a pattern
                ...
                try {
                    URL resourceURL = new File(getPathUntilWildcard(locationPattern.replaceFirst("file:", ""), true)).toURI().toURL();
