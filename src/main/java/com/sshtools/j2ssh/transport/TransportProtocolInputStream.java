...
class TransportProtocolInputStream {
  ...
  protected int readBufferedData(byte[] buf, int off, int len) throws
    ...
    if ( (endpos - startpos) < len) {
      ...
      /* jsiwek: I added some functionality to timeout the connection.
                 This is necessary because the client can enter an
                 infinite loop here (e.g. when not receiving a close or
                 disconnect message from a server in dedicated mode).
                 In that situation, reading from the socket does not block, so 
                 java.net.Socket.setSoTimeout() does not help, but we also do
                 not want to loop forever at the end of stream condition.
      */
      int timeWaited = 0;
      int maxWait = 20000;     // milliseconds
      int sleepDuration = 500; // milliseconds
        ...
        if (read < 0) {
            if(transport.getState().getValue()==TransportProtocolState.DISCONNECTED) {
                ...
                if (timeWaited > maxWait) {
                    throw new EOFException("Connection timed out.");
                }
                log.error("Buffer empty... EOFException suppressed");
                timeWaited += sleepDuration;
                try {
